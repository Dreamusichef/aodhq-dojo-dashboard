...
const PROD_CHANNELS = {
  ...
  lounge: '1343785602771980342',
  sentinelCouncil: '1367519122799464490',
...
// Channels whose per-author message counts feed each ninja's engagement fields.
//   stateKey   = dojo-state.json `channels[...]` cursor key (also the scan log label)
//   channelKey = key into the resolved `channels` map (PROD_CHANNELS / test config)
//   field      = the student-record field the running count lands in
// These are counted live every night (exactly like #practice-videos) and can be
// rebuilt from scratch via dev/recount-messages.js. Any channel with no configured
// id (e.g. in test mode) is skipped automatically.
const MESSAGE_CHANNELS = [
  { stateKey: 'the-hall',         channelKey: 'theHall',         field: 'hall' },
  { stateKey: 'lounge',           channelKey: 'lounge',          field: 'lounge' },
  { stateKey: 'sentinel-council', channelKey: 'sentinelCouncil', field: 'sentinel' },
];
