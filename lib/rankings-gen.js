...
const { normalizeLoc } = require('./public-projection');

// Keyed on BOTH the legacy raw values that live in dojo-data.json ('US', 'NY', 'Montreal',
// 'S. Africa') and the canonical country names LOC_MAP normalizes them to. The dashboard
// renders the normalized name while these rankings read the raw loc, so covering both keeps
// the two surfaces in agreement — a country present in one but not the other showed as 🏳️
// in Discord while the dashboard displayed it correctly (Canada, Indonesia, …).
const FLAGS = {
  ...
  // Canonical names produced by normalizeLoc (dashboard also shows these)
  'United States': '🇺🇸', 'United Kingdom': '🇬🇧', 'New Zealand': '🇳🇿',
  'South Africa': '🇿🇦', 'Tunisia': '🇹🇳',
  // Countries that had no flag at all
  'Canada': '🇨🇦', 'Sweden': '🇸🇪', 'Belgium': '🇧🇪', 'Indonesia': '🇮🇩',
  'Poland': '🇵🇱', 'Spain': '🇪🇸', 'Portugal': '🇵🇹', 'Ireland': '🇮🇪',
  'Denmark': '🇩🇰', 'Finland': '🇫🇮', 'Austria': '🇦🇹', 'Switzerland': '🇨🇭',
  'Mexico': '🇲🇽', 'Japan': '🇯🇵', 'Korea': '🇰🇷', 'South Korea': '🇰🇷',
  ...
  'Russia': '🇷🇺', 'Egypt': '🇪🇬', 'Morocco': '🇲🇦', 'Nigeria': '🇳🇬',
  'Pakistan': '🇵🇰', 'Bangladesh': '🇧🇩', 'Sri Lanka': '🇱🇰', 'Nepal': '🇳🇵',
  'Hong Kong': '🇭🇰', 'UAE': '🇦🇪', 'Saudi Arabia': '🇸🇦', 'Qatar': '🇶🇦',
};

// Try the raw value first (preserves the legacy keys exactly), then the canonical
// name LOC_MAP maps it to, so both spellings resolve to the same flag.
function flag(loc) {
  if (!loc) return '\u{1f3f3}\u{fe0f}';
  return FLAGS[loc] || FLAGS[normalizeLoc(loc)] || '\u{1f3f3}\u{fe0f}';
}
