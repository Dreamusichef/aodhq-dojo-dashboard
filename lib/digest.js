...
const { weekKeyToMonday } = require('./streaks');

const c = (code, text) => `\x1b[${code}m${text}\x1b[0m`;

const MON_NAMES = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
const dayLabel = (d) => `${MON_NAMES[d.getMonth()]} ${d.getDate()}`;

/**
 * Human dates for a week key, e.g. "Jul 27 – Aug 2". Recovery members must post in
 * their RETURN week specifically (rest-week posts are bonus and don't promote), so the
 * digest names the actual dates rather than a vague "before the window closes".
 */
function formatWeekRange(weekKey) {
  if (!weekKey) return null;
  const mon = weekKeyToMonday(weekKey);
  const sun = new Date(mon.getTime() + 6 * 86400000);
  return `${dayLabel(mon)} – ${dayLabel(sun)}`;
}
...
function buildDailyMessage(students, state, liveData) {
  ...
  // Streak watch: a MIX of the dojo's strongest live streaks (shown every day — a streak
  // is a weekly thing, alive all week even on non-post days) AND today's posters' streaks.
  // Today's qualifying posters are guaranteed a slot and tagged "· posted today"; the rest
  // of the (up to 6) slots go to the strongest live streaks overall. Qualify at Cycle 2+
  // (any week — a fresh "Cycle 2, Week 1" is the payoff, never hidden) or Cycle 1 Week 3+.
  const STREAK_CAP = 6;
  const qualifies = st => st && st.status === 'active' && (st.current_cycle >= 2 || st.current_week >= 3);
  const score = st => st.current_cycle * 100 + st.current_week;
  const byScore = (a, b) => score(b.st) - score(a.st);
  const postedToday = new Set(activeToday.map(x => x.s.u));

  const qualifying = students.map(s => ({ u: s.u, name: s.name, st: state.streaks[s.u] })).filter(x => qualifies(x.st));
  const todaysStreakers = qualifying.filter(x => postedToday.has(x.u)).sort(byScore);
  const otherStreakers = qualifying.filter(x => !postedToday.has(x.u)).sort(byScore);
  // Never drop a today-poster; fill remaining slots with the strongest others.
  const streakWatch = [...todaysStreakers, ...otherStreakers]
    .slice(0, Math.max(STREAK_CAP, todaysStreakers.length))
    .sort(byScore);

  const streakWatchLines = streakWatch.map(x =>
    `${c(33, '🔥 Streak watch:')} ${x.name} — Cycle ${c(36, String(x.st.current_cycle))}, Week ${c(36, String(x.st.current_week))}` +
    (postedToday.has(x.u) ? c(36, ' · posted today') : ''));

  if (N === 0) {
    lines.push('—');
    lines.push('Quiet day — 0 clips posted');
    lines.push('The Forge is always open.');
    if (streakWatchLines.length) { lines.push('—'); lines.push(...streakWatchLines); }
    lines.push('—');
  ...
  // Announce a cycle completion ONCE. Rest-week posts are encouraged bonus reps, and
  // every one of them used to re-fire this line — so a member who finished weeks ago got
  // re-announced as "just completed" each time they logged light reps.
  for (const { s } of activeToday) {
    const st = state.streaks[s.u];
    if (st && st.status === 'recovery' && st.current_week === 8 &&
        st.completion_announced !== st.current_cycle) {
  ...
  for (const s of students) {
    ...
    if (st && st.status === 'recovery') {
      ...
      const rw = formatWeekRange(st.recovery_deadline);
      lines.push(rw
        ? `🌀 ${s.name} — resting · post in your return week (${rw}) to begin Cycle ${nextCycle}`
        : `🌀 ${s.name} — resting · Cycle ${nextCycle} begins on your return`);
    }
  }
  ...
  if (allColdStart) {
    ...
    for (const { s, st } of toShow) {
      if (st.status === 'recovery') {
        const rw = formatWeekRange(st.recovery_deadline);
        lines.push(`${s.name} — resting${rw ? ` · return week ${rw}` : ''} → Cycle ${c(36, String(st.current_cycle + 1))}`);
      } else {
...
function formatMyStats(student, state, dojoTotal) {
  ...
  if (st && st.status === 'recovery') {
    const rw = formatWeekRange(st.recovery_deadline);
    lines.push(rw
      ? `🌀 Recovery — rest up, then post in your return week (${rw}) to begin Cycle ${st.current_cycle + 1}`
      : `🌀 Recovery (Cycle ${st.current_cycle + 1} begins on return)`);
