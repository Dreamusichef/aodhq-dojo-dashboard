...
const { milestoneFor, milestoneImagePath, buildCelebrationMessage } = require('./milestone');
...
function createPulseOps({ paths, discord, dryRun = false }) {
  ...
  function buildCelebration(milestone) {
    const data = loadDojoData(dataFile);
    const content = buildCelebrationMessage(data.students, milestone);
    const imgPath = milestoneImagePath(paths.workspace, milestone);
    return { content, imgPath, hasImage: fs.existsSync(imgPath) };
  }

  // Post the milestone celebration to #announcements. ping=false suppresses @everyone (tests).
  async function postCelebration(client, milestone, { ping = true } = {}) {
    const { content, imgPath, hasImage } = buildCelebration(milestone);
    if (dryRun) { console.log('[DRY RUN] Would post milestone', milestone, 'to #announcements (image:', hasImage, ', ping:', ping, ')'); return; }
    const channel = await client.channels.fetch(channels.announcements);
    await channel.send({
      content: '@everyone\n\n' + content,
  ...
  // Auto-detector: fire ONCE when the dojo total crosses a new 1,000 boundary (>= 2,000)
  // AND that milestone's image exists. Holds (no post) until the image is in place.
  async function runMilestoneCheck(client) {
    ...
    if (!fs.existsSync(milestoneImagePath(paths.workspace, m))) {
      console.log('[Milestone] ' + m + ' reached but assets/milestone-' + m + '.png missing — holding.');
      return;
    }
    await postCelebration(client, m, { ping: true });
    state.last_milestone = m;
    saveState(state);
    console.log('[Milestone] Celebrated ' + m + ' in #announcements.');
  }
