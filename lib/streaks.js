...
/**
 * Exact inverse of sgtWeekKey: the Monday that starts the given week key.
 *
 * sgtWeekKey numbers a week as ceil((daysFromJan1 + jan1.getDay() + 1) / 7), which does
 * NOT make the year's first Monday week 1 — e.g. Jan 1 2026 is a Thursday, so the first
 * Monday (Jan 5) is W02. Assuming "first Monday = W01" shifted every rendered date a week
 * late, so digests advertised recovery return weeks 7 days after the real deadline.
 * Anchor off the first Monday's ACTUAL sgtWeekKey number instead.
 */
function weekKeyToMonday(weekKey) {
  ...
  const rawDay = jan1.getDay();               // 0=Sun..6=Sat — same basis sgtWeekKey uses
  const daysToFirstMonday = (8 - (rawDay || 7)) % 7;
  const firstMonday = new Date(jan1);
  firstMonday.setDate(jan1.getDate() + daysToFirstMonday);
  // The week number sgtWeekKey assigns to that first Monday.
  const firstMondayWeekNo = Math.ceil((daysToFirstMonday + rawDay + 1) / 7);
  const result = new Date(firstMonday);
  result.setDate(firstMonday.getDate() + (week - firstMondayWeekNo) * 7);
  return result;
}
 ...
 *  - Recovery: 2 full rest weeks (W1, W2 — the "14 days of rest") + the RETURN week
 *    (W3, through Sunday). recovery_deadline = W0 + 3 = W3.
 *  - Rest-week posts (W1, W2) are bonus light reps: they never promote and never start
 *    the next cycle's clock — no obligation resumes during rest.
 *  - RETURN: a post IN W3 promotes to the next cycle, Week 1 (= W3) — the "Cycle N+1
 *    begins on return" promise. Judged by the week the POST landed in, never by the
 *    night it is processed, so a Sunday-evening return processed on Monday still counts.
 *  - No post in W3 → true reset (bonus rest posts don't save it); the next post starts
 *    over at Cycle 1, Week 1.
...
function computeStreaks(students, streaks) {
  ...
  for (const student of students) {
    ...
    if (s.status === 'recovery') {
      // Rest-week posts (W1, W2) are BONUS light reps — they never promote and never
      // start the next cycle's clock. Promotion requires a post in the RETURN week
      // (W3 = recovery_deadline), any day through Sunday; judged by the week the post
      // landed in, so a late-Sunday return processed on Monday still counts.
      const returnWeek = s.recovery_deadline;
      const returned = returnWeek && activeWeeks.has(returnWeek);

      if (returned) {
        // The promise, honored: the next cycle's Week 1 is the return week.
        s.carry = s.current_cycle || 0;
        s.current_cycle = s.carry + 1;
        s.current_week = 1;
        s.status = 'active';
        s.cycle_start_date = returnWeek;
        s.cycle_anchor_week = addWeeks(returnWeek, -1); // walk counts from W3 onward
